CLASS zcl_ey_AS_amdp DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
"  class - this class is now ready to contain SQL script code
    ...
    CLASS-METHODS get_customer_by_id IMPORTING
                                        value(i_bp_id) TYPE zey_AS_dte_id
                                     EXPORTING
                                        VALUE(e_res) TYPE char256.
    CLASS-METHODS get_product_mrp IMPORTING
                                    VALUE(i_tax) type i
                                  EXPORTING
                                    VALUE(otab) type zey_AS_tt_product_mrp.
    CLASS-METHODS get_total_sales for table FUNCTION ZEY_AS_TF.
  PROTECTED SECTION.
  PRIVATE SECTION.
ENDCLASS.
...
CLASS ZCL_EY_AS_AMDP IMPLEMENTATION.
  ...
  METHOD get_customer_by_id BY DATABASE PROCEDURE FOR HDB LANGUAGE SQLSCRIPT
    ...
    select concat( 'M/s ', company_name ) as company_name into e_res from zey_AS_bpa
     where bp_id = :i_bp_id ;
...
*   get all the products in a implicit table (like a internal table in abap)
    lt_prod = select * from zey_AS_product;
...
*   fill the otab for result (like in abap we fill another internal table with data)
  ...
  METHOD get_total_sales by DATABASE FUNCTION FOR HDB LANGUAGE SQLSCRIPT
                        OPTIONS READ-ONLY
                        USING zey_AS_bpa zey_AS_so_hdr zey_AS_so_item
  .
    ...
    return select
     ...
     from zey_AS_bpa as bpa
    INNER join zey_AS_so_hdr as sls
    on bpa.bp_id = sls.buyer
    inner join zey_AS_so_item as item
...
*        zcl_ey_AS_amdp=>get_customer_by_id(
*          EXPORTING
...
*        ).
        ...
        zcl_ey_AS_amdp=>get_product_mrp(
          EXPORTING
            i_tax = 18
          IMPORTING
            otab  = data(result)
